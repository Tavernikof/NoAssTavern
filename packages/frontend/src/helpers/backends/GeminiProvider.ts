...
class GeminiProvider extends BaseBackendProvider {
    ...
    const {
      ...
      parseJson: (data) => {
        ...
        if (key === "promptFeedback") {
          ...
          response.error = "finishReason: " + value?.blockReason || "unknown reason";
        }
        if (key === "candidates") {
        ...
        if (key === "usageMetadata") {
          ...
          response.inputTokens = value.promptTokenCount;
          response.outputTokens = value.candidatesTokenCount;
        }
  ...
  getModelsOptions(connectionProxy?: ConnectionProxy): Promise<BackendProviderGetModelsResponse> {
    ...
    const isDefaultEndpoint = baseUrl === this.baseUrl;
    return backendManager.externalRequest<GeminiModelsResponse>({
      method: "GET",
