...
export class ChatFormContextStore {
  ...
  @computed
  get flowsOptions() {
    const flows = flowsManager.fullList.map(flow => ({
      value: flow.id,
      label: flowsManager.getLabel(flow),
    }));
  ...
  get loreBookOptions() {
    ...
    loreBookManager.fullList.forEach(loreBook => loreBooksOptions.push({
      value: loreBook.id,
      label: loreBookManager.getLabel(loreBook),
    }));
