...
export const filesRoutes = (storage: StorageService) => async (app: ZodFastifyInstance) => {

  app.register(staticServe, {
    root: storage.files.filesDir,
    prefix: "/",
  });

  app.get("/:id", { schema: { params: paramsSchema } }, async (req, reply) => {
    const { id } = req.params;
    const matches = await glob(`${id}.*`, { cwd: storage.files.filesDir });
  ...
  app.post("/:id", { schema: { params: paramsSchema } }, async (req, reply) => {
    ...
    const ext = path.parse(file.filename || "").ext || ".bin";
    const buffer = await file.toBuffer();
    await storage.files.create(id, buffer, ext);
  });

  app.post("/:existId/clone/:newId", { schema: { params: cloneParamsSchema } }, async (req, reply) => {
    const { existId, newId } = req.params;
    await storage.files.clone(existId, newId);
  });

  app.delete("/:id", { schema: { params: paramsSchema } }, async (req) => {
    const { id } = req.params;
    const ok = await storage.files.removeFile(id);
    return { success: ok };
  });
};
