...
- Каждый код-блок представляет собой файл с кодом, в котором должны быть определены глобальные функции с определенными
  именами
- Код запускается в изолированном iframe, так что не имеет доступа к контексту страницы
...
На данный момент реализовано 4 хука:
...
| `preRequest`    | Модифицирует запрос к LLM непосредственно перед отправкой. Принимает на вход тело запроса, url, заголовки и тип провайдера (`openai`/`gemini`/`claude`).                               |
...
| `formatMessage` | Модифицирует то, как отображается сообщение, не меняя исходный текст. Тут можно возвращать html и css, которые будут применены к сообщению.                                            |

## Пример

![](/code-block-probiv.png){width=300px}

На скриншоте выше представлен код-блок, который реализует один из подходов для
обхода [внешнего фильтра для Gemini](https://rentry.org/gemini-filters). Как он работает:
